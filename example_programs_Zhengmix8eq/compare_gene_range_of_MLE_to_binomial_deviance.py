...
filtered_UMI_counts_pkl = 'filtered_UMI_counts.pkl'
...
binomial_deviance_saturated_all_genes_dsn  = data_path / binomial_deviance_saturated_all_genes_pkl 

filtered_UMI_counts_pkl_dsn = data_path / filtered_UMI_counts_pkl 
...
df_filtered_UMI_counts  = pd.read_pickle ( filtered_UMI_counts_pkl_dsn )
...
df_analy_filtered_genes = df_analy.loc [ df_filtered_UMI_counts.index ]

plog ( logfile, '\n\n df_analy_filtered_genes:\n' , df_analy_filtered_genes )
plog ( logfile, '\n\n df_analy_filtered_genes.describe:\n' , df_analy_filtered_genes[[ 'pi_hat_max', 'pi_hat_min', 'pi_hat_range', 'Log10_pi_hat_range', 'Log10_saturated_bin_dev' ]].describe ( percentiles = pctl_list ) )
  
  

title1 = 'Zhengmix8eq data - all genes: ' + str ( clustering ) + ' cluster model'
title2 = "\n compare range of each gene's cluster MLEs to binomial deviance"
title3 = '\n filtered genes are highlighted in red'
...
ax.scatter ( df_analy_filtered_genes ['Log10_saturated_bin_dev'], df_analy_filtered_genes['Log10_pi_hat_range']  , s=1.0, color='red'  )
